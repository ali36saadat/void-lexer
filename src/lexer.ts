...
let tool = {
   ...
   isInArray(needle: any, haystack: any) {
      return haystack.indexOf(needle) > -1
   },
   judgeTokenTypeByValue(value: any) {
   ...
   judgeTokenType(state: any, value: any) {
...
let flowModel = {
   ...
   getNextState(ch: any, state: any, matchs: any) {
...
let lexer: any = {
   ...
   CFG: {
      ignoreTokens: [],
      ignoreValues: [ENUM_CONST.WHITESPACE],
   ...
   ISR: {
      ...
      after() {
         ...
         // console.log(lexer.DFA.result.tokens)
      },
   ...
   DFA: {
      ...
      resultChange: {
         ...
         filterTokens() {
            let tokens: any[] = []
            lexer.DFA.result.tokens.forEach((token: any) => {
               if (
                  lexer.CFG.ignoreValues.indexOf(token.value) < 0 &&
                  lexer.CFG.ignoreTokens.indexOf(token.type) < 0
               ) {
                  tokens.push(token)
               }
            })
      ...
      events: {
         flowtoNextState(ch: any, state: any) {
...
const LEXER = function (value: any) {
   // const RES: any = value.length != 0 ? lexer.start(value) : "ERROR"
